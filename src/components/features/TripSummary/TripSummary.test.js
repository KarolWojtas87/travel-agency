...
describe('Component TripSummary', () => {
  ...
  it('should render tags array', () => {
    ...
    expect(component.find('.tags .tag').at(0).text()).toEqual('beach');
    expect(component.find('.tags .tag').at(1).text()).toEqual('pool');
    expect(component.find('.tags .tag').at(2).text()).toEqual('spa');
  });

  it('should not be rendered if tags array is falsy', () => {
    const expectedTags = [];
    const component = shallow(<TripSummary tags={expectedTags} />);
    expect(component.find('.tags .tag').length).toEqual(0);
  });
